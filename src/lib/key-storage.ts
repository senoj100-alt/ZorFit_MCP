 ...
 * 
 * Provides secure storage and retrieval of user service credentials in Cloudflare KV.
...
/**
 * Store a user's service credential in KV (encrypted).
 */
export async function setUserServiceCredential(
	kv: KVNamespace,
	encryptionKey: string,
	username: string,
	service: string,
	...
	const encrypted = await encryptApiKey(credential, encryptionKey);
	const kvKey = `service_credential:${service}:${username}`;
	await kv.put(kvKey, encrypted);
}
...
/**
 * Retrieve a user's service credential from KV (decrypted).
 */
export async function getUserServiceCredential(
	kv: KVNamespace,
	encryptionKey: string,
	username: string,
	service: string,
	...
	try {
		...
		console.error(
			`Failed to decrypt ${service} credential for user ${username}:`,
			error,
		);
		return null;
	}
...
/**
 * Delete a user's service credential from KV.
 */
export async function deleteUserServiceCredential(
	kv: KVNamespace,
	username: string,
	service: string,
): Promise<void> {
